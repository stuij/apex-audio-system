...
static void AAS_DoConfig(int mix_rate, int volscale, AAS_BOOL stereo,
                         AAS_BOOL dynamic, AAS_BOOL loud, int chans) {
  ...
  if (!AAS_initialised) {
    ...
    REG_DMA2CNT_H = 0xb600; // dma control: DMA enabled+ start on
                            // FIFO+32bit+repeat+increment source&dest
  ...
  if (stereo) {
    ...
    REG_SOUNDCNT_H = 0x9a0d; // enable DS A&B + fifo reset + use timer0 + 100%
                             // volume to L and R
  } else {
    int a, b;
    ...
    REG_SOUNDCNT_H = 0xbb0d; // enable DS A&B + fifo reset + use timer0 + 100%
                             // volume to L and R
  }
...
int AAS_GetOutputBufferLength() { return AAS_next_mix_rate * 16; }
...
void AAS_FastTimer1InterruptHandler() {
  ...
  REG_DMA1CNT_H = 0xb600; // DMA control: DMA enabled+start on
                          // FIFO+32bit+repeat+increment source&dest
  // Get click on hardware when switch off DMA on Direct Sound B, so have to do
  // it this way instead
  if (AAS_db_active)
      ...
      if (AAS_Loud) {
        if (AAS_DSA_first) {
          ...
          if (tmp1) {
            ...
            if (AAS_changed[0]) {
              AAS_INIT_BRANCH
              AAS_BRANCH(AAS_MixAudio, AAS_next_mixing_buffer, &AAS_channels[0],
                         curr_mix_rate);
            } else {
              AAS_INIT_BRANCH
              AAS_BRANCH(AAS_MixAudio_NoChange, AAS_next_mixing_buffer,
                         &AAS_channels[0], curr_mix_rate);
            }
          ...
          if (tmp2) {
            ...
            {
              AAS_INIT_BRANCH
              AAS_BRANCH(AAS_MixAudio, AAS_next_mixing_buffer + 640,
                         &AAS_channels[8], curr_mix_rate);
            }
          ...
          if (tmp2) {
            ...
            if (AAS_changed[1]) {
              ...
              AAS_INIT_BRANCH
              AAS_BRANCH(AAS_MixAudio_NoChange, AAS_next_mixing_buffer + 640,
                         &AAS_channels[8], curr_mix_rate);
            }
            AAS_db_active = AAS_TRUE;
          } else {
...
int AAS_GetActualMixRate() { return AAS_next_mix_rate * 800; }
