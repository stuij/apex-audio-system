...
int AAS_MOD_GetNumChannels() { return AAS_mod_num_chans; }

int AAS_MOD_GetLastFilterValue() { return AAS_mod_last_filter_value; }
        ...
        for (chan = 0; chan < AAS_mod_num_chans; ++chan) {
          mod_chan->pattern =
              (AAS_u32 *)(AAS_PatternData +
                          (((int)AAS_Sequence[AAS_mod_num][song_pos][chan])
                           << 8));
          ++mod_chan;
        }
        AAS_mod_line_num = 0;
...
int AAS_MOD_GetVolume() { return AAS_mod_overall_volume; }
...
int AAS_MOD_SetVolume(int vol) {
  if ((vol >= 0) && (vol <= 256)) {
    ...
    for (i = AAS_mod_num_chans - 1; i >= 0; --i)
      AAS_channels[AAS_chan_rearrange[i]].volume =
          (AAS_mod_chan[i].volume * vol) >> AAS_volscale;
...
AAS_BOOL AAS_MOD_HasLooped() { return AAS_mod_looped; }

AAS_BOOL AAS_MOD_IsPlaying() { return (AAS_mod_num >= 0); }

int AAS_MOD_Play(int song_num) {
  AAS_MOD_Stop();

  if (AAS_initialised) {
    if ((song_num >= 0) && (song_num < AAS_DATA_NUM_MODS) &&
        AAS_NumChans[song_num]) {
      int i;
      ...
      if (AAS_NumChans[song_num] > i) {
        ...
        for (i = 0; i < AAS_mod_num_chans; ++i)
          AAS_mod_chan[i].pattern =
              (AAS_u32 *)(AAS_PatternData +
                          (((int)AAS_Sequence[song_num][0][i]) << 8));
        ...
        for (chan = 0; chan < num_chans; ++chan) {
          ...
          if (effect) {
            ...
            case 0xe: // extended effects
              ...
              case 0xd0: // delay sample
                if (mod_chan->trigger >= 0) {
                  --mod_chan->trigger;
                  if (mod_chan->trigger < 0) {
                    const struct AAS_ModSample *samp =
                        &AAS_ModSamples[AAS_mod_num][mod_chan->samp_num];
                    ...
                    out_chan->frequency =
                        AAS_MOD_period_conv_table[mod_chan->period];
                    out_chan->delta = AAS_Min(
                        4095,
                        ((out_chan->frequency * AAS_mix_scale) + 32768) >> 16);
            ...
            case 0x6: // vibrato + volume slide
              ...
              tmp = mod_chan->period +
                    (((AAS_sin[val] * mod_chan->vibrato_depth) + 32) >> 6);
              if (tmp < 113)
                tmp = 113;
              if (tmp > 856)
                tmp = 856;
              ...
              out_chan->delta = AAS_Min(
                  4095, ((out_chan->frequency * AAS_mix_scale) + 32768) >> 16);
            ...
            case 0x7: // tremolo
              ...
              tmp = mod_chan->volume +
                    (((AAS_sin[val] * mod_chan->tremolo_depth) + 32) >> 6);
              if (tmp < 0)
                tmp = 0;
              else if (tmp > 64)
                tmp = 64;
            ...
            case 0x0: // possible arpeggio
              ...
              if (tmp) // definite arpeggio
              {
                ...
                case 2:
                  val += tmp & 0xf;
                  // Intentionally no "break;" to allow
                  // AAS_mod_arpeggio_pos[chan] to be restarted
                ...
                if (val) {
                  out_chan->frequency = AAS_MOD_period_conv_table
                      [AAS_period_table[AAS_ModSamples[AAS_mod_num]
                                                      [mod_chan->samp_num]
                                                          .finetune][val - 1]];
                  out_chan->delta = AAS_Min(
                      4095,
      ...
      for (chan = 0; chan < num_chans; ++chan) {
        ...
        if (effect) {
          if (effect < 0x100) {
            ...
            if (tmp) {
              out_chan->frequency = AAS_MOD_period_conv_table
                  [AAS_period_table[mod_samp[mod_chan->samp_num].finetune]
                                   [tmp - 1]];
              out_chan->delta = AAS_Min(
                  4095, ((out_chan->frequency * AAS_mix_scale) + 32768) >> 16);
        ...
        if (effect) {
          ...
          case 0xf: // set speed
            ...
            if (speed > 0) {
              ...
              // AAS_mod_tempo =
              // (((1<<15)*AAS_mod_bpm*24)/(3000*AAS_mod_speed))<<13;  // use LUT
      ...
      if (jump_ahead >= 0) {
        ...
        for (chan = 0; chan < num_chans; ++chan) {
          mod_chan->pattern =
              (AAS_u32
                   *)(AAS_PatternData +
                      (((int)AAS_Sequence[AAS_mod_num][AAS_mod_song_pos][chan])
                       << 8));
        ...
        if (AAS_mod_line_num > 63) {
          ...
          for (chan = 0; chan < num_chans; ++chan) {
            mod_chan->pattern =
                (AAS_u32 *)(AAS_PatternData +
                            (((int)AAS_Sequence[AAS_mod_num][AAS_mod_song_pos]
                                               [chan])
                             << 8));
            ++mod_chan;
          }
